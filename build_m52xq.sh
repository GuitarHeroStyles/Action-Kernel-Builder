...
PACK_DTBO_IMG()
{
    ...
    #cp "$WORK_DIR/target/m52xq/images/$IMG_FOLDER/dtbo.img" "$OUT_DIR/out/zip/mesa/$IMG_FOLDER/dtbo.img"
...
MAKE_INSTALLER()
{
    cp -r "$WORK_DIR/target/m52xq/template/META-INF" "$OUT_DIR/out/zip/META-INF"
...
for i in eur swa cis; do
    rm -f "$WORK_DIR/target/m52xq/images/$i/*.img"
done
